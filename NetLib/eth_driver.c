 ...
 */
#if(PHY_MODE ==  USE_MAC_MII)
void ETHDRV_QueryPhyState(void)
{
    u16 phy_stat;
    if(QUERY_STAT_FLAG)
    {                                         /* Query the PHY link status every 1s */
...
#if( PHY_MODE ==  USE_MAC_MII )
    ETHDRV_QueryPhyState();                   /* Query external PHY status*/
#endif
}
